...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
   ...
   const updateAccess = useCheckPermissions(user, ['product:update']);
   const deleteAccess = useCheckPermissions(user, ['product:delete']);
            ...
            <DropdownMenuContent align="end">
               ...
               {updateAccess && (
                  <DropdownMenuItem
                     className="flex cursor-pointer items-center gap-2"
                     onClick={() =>
                        router.push(`/dashboard/product/update?id=${data.id}`)
                     }
                     disabled={loading}
                  >
